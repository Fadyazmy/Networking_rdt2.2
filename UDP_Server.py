...
while True:
    ...
    if not corrupt(UDP_Packet):
        ...
        chksum = make_checksum(UDP_Packet[1],UDP_Packet[1],'')

        # Build the UDP Packet [CURR_ACK, CURR_SEQ, data, chksum]
        UDP_Packet = make_pkt(UDP_Packet[0], UDP_Packet[1], '', chksum)
        print('Packeting')
        ...
        #Built checksum [ACK, SEQ, DATA]
        chksum = make_checksum(UDP_Packet[1],(UDP_Packet[1] +1)%2,'')

        # Build the UDP Packet [CURR_ACK, CURR_SEQ, data, chksum]
        UDP_Packet = make_pkt(UDP_Packet[0], (UDP_Packet[1] +1)%2, '', chksum)
        print('Packeting')
